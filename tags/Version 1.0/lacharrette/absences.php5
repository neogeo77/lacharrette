...
<br><br><h1>MES ABSENCES - <?php echo $utilisateur->PrenomNOM();?> - <?php echo $calendrier->trajet->titre;?></h1><br><br>
<br><br><br>

<form method="POST" action="absences_trt.php5">
...
for ($debut=$joursAvant;$debut<$joursApres;$debut+=$joursParLigne)
{
	...
	for ($jour=$debut;$jour<$debut+$joursParLigne && $jour<$joursApres;$jour++)
	{
		...
		{
			$code = $calendrier->getCode($date, $trigrammeUser);
